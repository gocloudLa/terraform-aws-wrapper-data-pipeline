    ...
    [
      for glue_job_key, glue_job_config in try(data_pipeline_config.jobs, {}) :
      {
        "${data_pipeline_key}-${glue_job_key}" = {
          ...
          create_parameter_store      = try(glue_job_config.create_parameter_store, false)
          custom_parameter_store_name = try(glue_job_config.custom_parameter_store_name, null)
