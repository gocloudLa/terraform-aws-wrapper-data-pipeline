...
# Ref: https://registry.terraform.io/providers/hashicorp/aws/latest/docs/resources/glue_crawler
resource "aws_glue_crawler" "this" {
  ...
  role                   = try(aws_iam_role.glue_crawler[0].arn, null)
...
resource "aws_iam_role" "glue_crawler" {
  ...
  name        = "${var.name}-role"
  description = "Role for crawler ${var.name}"
  tags        = var.tags
